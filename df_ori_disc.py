    ...
    for i in range(num_ori):
        ...
        if 6 < i < 16:
            for ii in range(num):
                ...
                dataset[i * num * 2 + ii, :, :, :] = -rp.df_representation(
                    ref, img[0])
                dataset[i * num * 2 + ii + num, :, :, :] = -rp.df_representation(
                    ref, img[1])
        else:
            for ii in range(num):
                label[i * num * 2 + ii] = 1
...
def ff_train(show_epc, net_name, slow_learning=True, bg_epoch=0):
    ...
    for i in range(num_test // 2):
        ...
        t_inputs[2 * i, :, :, :] = rp.df_representation(ref, img_tg[0])
        t_inputs[2 * i + 1, :, :, :] = rp.df_representation(ref, img_tg[1])

        t_labels[1 * num_test + 2 * i] = 1
        t_labels[1 * num_test + 2 * i + 1] = -1
        img_tg = Img_DT.gen_test()
        ref = Img_DT.gen_reference()
        t_inputs[1 * num_test + 2 * i, :, :, :] = -rp.df_representation(
            ref, img_tg[0])
        t_inputs[1 * num_test + 2 * i + 1, :, :, :] = -rp.df_representation(
            ref, img_tg[1])
        ...
        t_inputs[2 * num_test + 2 * i, :, :, :] = rp.df_representation(
            ref, img_tg[0])
        t_inputs[2 * num_test + 2 * i + 1, :, :, :] = rp.df_representation(
            ref, img_tg[1])
        ...
        t_inputs[3 * num_test + 2 * i, :, :, :] = -rp.df_representation(
            ref, img_tg[0])
        t_inputs[3 * num_test + 2 * i + 1, :, :, :] = -rp.df_representation(
            ref, img_tg[1])
    ...
    for i in range(5):
        ...
        t_ref[i, :, :, :] = rp.df_representation(img_ref, img_tg[0])
        t_ref[5 + i, :, :, :] = rp.df_representation(img_ref, img_tg[1])
        t_ref[10 + i, :, :, :] = rp.df_representation(img_ref, img_ref)
    size_input = t_ref.shape
    t_ref = np.reshape(
        tools_PL.normalize(np.reshape(t_ref, [size_input[0], -1])), size_input)
    ...
    for epoch in range(200):  # loop over the dataset multiple times
        ...
        for i in range(num_batch):
            ...
            ref = Img_L.gen_reference()
            inputs[i, :, :, :] = rp.df_representation(ref, img_tg)
...
for net_name in net_list:
    for begin_epoch in [199]:
        ...
        name_head = foldername + '/New/df_ori_dis_asys' + net_name
