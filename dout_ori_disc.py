...
def GenTestData(_ori, num=10, loc="L", diff=0):
    ...
    refset = np.zeros([num * 2 * num_ori, 1, 40, 18])
    ...
    for i in range(num_ori):
        ...
        for ii in range(num):
            ...
            img = Img.gen_test()
            ref = Img.gen_reference()
            dataset[i * num * 2 + ii, :, :, :] = rp.representation(img[0])
            dataset[i * num * 2 + ii + num, :, :, :] = rp.representation(img[1])
            refset[i * num * 2 + ii, :, :, :] = rp.representation(ref)
            refset[i * num * 2 + ii + num, :, :, :] = rp.representation(ref)
    return dataset, label, refset
...
def ff_train(show_epc, net_name, slow_learning=True, bg_epoch=0):
    ...
    Sti = stimulus.Gabor(sigma=30, freq=0.01)
    Img_L = rp.GenImg(Sti, orient=20, loc="L", diff=0)
    Img_R = rp.GenImg(Sti, orient=20, loc="R", diff=0)
    Img_DT = rp.GenImg(Sti, orient=110, loc="R", diff=0)
    Img_th = rp.GenImg(Sti, orient=110, loc="L", diff=0)
    inputs = np.zeros([num_batch, 1, 40, 18])
    inp_refere = np.zeros([num_batch, 1, 40, 18])
    ...
    t_reference = np.zeros([num_test * 4, 1, 40, 18])
    ...
    t_inputs = np.reshape(
        ...
        size_input)
    t_reference = np.reshape(
        tools_PL.normalize(np.reshape(t_reference, [size_input[0], -1])),
        size_input)
    acc_list[0, 1:] = test(net, t_inputs, t_reference, num_test, t_labels)[:-1]
    acc_list[0, 0] = acc_list[0, 1]
    best_acc = np.zeros(np.size(acc_list[0, :]))

    ## generate reference

    t_ref = np.zeros([20, 1, 40, 18])
    for i in range(5):
        img_tg = Img_L.gen_test()
        ...
        t_ref[15 + i, :, :, :] = rp.representation(img_ref) #拆成两个矩阵丢到里面去 TODO
    ...
    for epoch in range(200):  # loop over the dataset multiple times
        ...
        for i in range(num_batch):
            ...
            inp_refere[i, :, :, :] = rp.representation(ref)

        #这部分主要是有一个ff干扰，不能整合起来
        ...
        b_x = torch.tensor(inp_refere,dtype=torch.float32)
        out_ref = net(b_x)
        out_ref = out_ref.squeeze(1)

        loss = criterion(outputs-out_ref, torch.FloatTensor((labels + 1) // 2))
        # pdb.set_trace()
        loss.backward()
        optimizer.step()
        ...
        if epoch % show_epc == show_epc - 1:  # print every show epoch
            print_test = False
            acc = tools_PL.np_acc(outputs-out_ref, labels)
            acc_list[_epc, 0] = acc
            ...
            t_input_ff = np.reshape(
                ...
                size_input)
            t_reference_ff = feedforward(t_reference,weight)
            t_reference_ff = np.reshape(tools_PL.normalize(np.reshape(t_reference_ff, [size_input[0], -1])),size_input)
            acc_test = test(net, t_input_ff, t_reference_ff, num_test, t_labels, print_test)
...
def test(net, inputs, refers, num_batch, labels, prt=False):
    with torch.no_grad():
        b_x = torch.tensor(inputs, dtype=torch.float32)
        outputs = net(b_x)
        outputs = outputs.squeeze(1)
        b_x = torch.tensor(refers, dtype=torch.float32)
        outref = net(b_x)
        outref = outref.squeeze(1)
        acc = ACC(outputs-outref, labels, num_batch)
        if prt:
...
def reference(net, input):
    with torch.no_grad():
        ...
        print(torch.sign(outputs[:10]-outputs[10:]))
        print('###################')
...
print_test = False
net_list = ['s_cc3']
t_data, t_label, t_ref = GenTestData(_ori=ori)
size_input = t_data.shape
t_data = np.reshape(tools_PL.normalize(np.reshape(t_data, [size_input[0], -1])),
                    size_input)
t_ref = np.reshape(tools_PL.normalize(np.reshape(t_ref, [size_input[0], -1])),
                    size_input)
# pdb.set_trace()
        ...
        for s in range(num_section):
            ...
            for i in range(40 // loc):
                ...
                y = np.reshape(
                    ...
                    size_input)
                y_ref = feedforward(t_ref,weight)
                y_ref = np.reshape(
                    tools_PL.normalize(np.reshape(y_ref, [size_input[0], -1])),
                    size_input)

                # plt.subplot(2, 4, i+1)
                # plt.imshow(y[1].squeeze())
                Acc_test[s, i, :] = test(net, y, y_ref, num_test * 2, t_label,
                                         print_test)[:-1]
                Acc_test_fx[s, i, :] = test(bestNet, t_data, t_ref, num_test * 2,
                                            t_label,
                                            print_test)[:-1]
            ...
            #         y_ref = np.reshape(
            #             tools_PL.normalize(
            #                 np.reshape(y_ref, [size_input[0], -1])),
            #             size_input)
            #         # plt.subplot(2, 4, i+1)
            #         # plt.imshow(y[1].squeeze())
            #         Acc_test_db[s, db_i, i, :] = test(net, y, y_ref, num_test * 2,
            #                                              t_label,
            #                                              print_test)[:-1]
            #         t_data = np.roll(t_data, -5, axis=2)
