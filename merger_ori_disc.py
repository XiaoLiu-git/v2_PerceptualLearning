...
def GenTestData(_ori, num=10, loc="L", diff=0):
    ...
    dataset = np.zeros([num * 2 * num_ori, 1, 40, 36])
    ...
    for i in range(num_ori):
        ...
        for ii in range(num):
            ...
            dataset[i * num * 2 + ii, :, :, :] = rp.merger_representation(
                ref, img[0])
            dataset[i * num * 2 + ii + num, :, :, :] = rp.merger_representation(
                ref, img[1])
...
def ff_train(show_epc, net_name, slow_learning=True, bg_epoch=0):
    ...
    inputs = np.zeros([num_batch, 1, 40, 36])
    labels = np.zeros([num_batch])

    # network
    if net_name is "s_cc3":
        net = nw.Net_sCC_merger()
    ...
    optimizer = optim.Adam([
        ...
        {'params': net.layer1.parameters(), 'lr': 1e-2}
    ], lr=1e-2)
    running_loss = 0.0
    ...
    t_inputs = np.zeros([num_test * 4, 1, 40, 36])
    ...
    for i in range(num_test // 2):
        ...
        t_inputs[2 * i, :, :, :] = rp.merger_representation(ref, img_tg[0])
        t_inputs[2 * i + 1, :, :, :] = rp.merger_representation(ref, img_tg[1])

        t_labels[1 * num_test + 2 * i] = 1
        t_labels[1 * num_test + 2 * i + 1] = -1
        img_tg = Img_DT.gen_test()
        ref = Img_DT.gen_reference()
        t_inputs[1 * num_test + 2 * i, :, :, :] = rp.merger_representation(
            ref, img_tg[0])
        t_inputs[1 * num_test + 2 * i + 1, :, :, :] = rp.merger_representation(
            ref, img_tg[1])
        ...
        t_inputs[2 * num_test + 2 * i, :, :, :] = rp.merger_representation(
            ref, img_tg[0])
        t_inputs[2 * num_test + 2 * i + 1, :, :, :] = rp.merger_representation(
            ref, img_tg[1])
        ...
        t_inputs[3 * num_test + 2 * i, :, :, :] = rp.merger_representation(
            ref, img_tg[0])
        t_inputs[3 * num_test + 2 * i + 1, :, :, :] = rp.merger_representation(
            ref, img_tg[1])
    ...
    # Training
    weight = np.ones([40, 36])
    for epoch in range(200):  # loop over the dataset multiple times

        for i in range(num_batch):
            labels[i], img_tg = Img_L.gen_train()
            ref = Img_L.gen_reference()
            inputs[i, :, :, :] = rp.merger_representation(ref, img_tg)
...
for net_name in net_list:
    for begin_epoch in [199]:
        for s in range(num_section):
            ...
            for i in range(40 // loc):
                ...
                Acc_test[s, i, :] = test(net, y, num_test * 2, t_label,
                                         print_test)[:-1]
                Acc_test_fx[s, i, :] = test(bestNet, t_data, num_test * 2,
                                            t_label,
                                            print_test)[:-1]
        ...
        name_head = foldername + '/New/merg_ori_dis' + net_name
