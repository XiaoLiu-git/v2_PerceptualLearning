...
def GenTestData(_ori, num=10, loc="L", diff=10):
    num_ori = int(180 / _ori)
    dataset = np.zeros([num * 2 * num_ori, 1, 40, 18])
    label = np.zeros([num * 2 * num_ori])
    for i in range(num_ori):
        ori_i = i * _ori
        Sti = stimulus.Gabor(sigma=20, freq=0.01)
        Img = rp.GenImg(Sti, orient=ori_i, loc=loc, diff=diff, noise_cutout=0.4)
        for ii in range(num):
            label[i * num * 2 + ii] = 1
            label[i * num * 2 + ii + num] = -1
            img = Img.loc_gen_test()
...
def ff_train(show_epc, net_name, slow_learning=True, bg_epoch=0):
    ...
    Sti = stimulus.Gabor(sigma=20, freq=0.01)
    Img_L = rp.GenImg(Sti, orient=20, loc="L", diff=5, noise_cutout=0.4)
    Img_R = rp.GenImg(Sti, orient=20, loc="R", diff=5, noise_cutout=0.4)
    Img_DT = rp.GenImg(Sti, orient=110, loc="R", diff=5, noise_cutout=0.4)
    Img_th = rp.GenImg(Sti, orient=110, loc="L", diff=5, noise_cutout=0.4)
    ...
    for epoch in range(200):  # loop over the dataset multiple times
        ...
        for i in range(num_batch):
            labels[i], img_tg = Img_L.loc_gen_train()
...
for net_name in net_list:
    for begin_epoch in [199]:
        ...
        name_head = foldername + '/New/loc_dis' + net_name
